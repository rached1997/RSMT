...
class SurpriseAdequacy(ABC):
    ...
    def _get_saved_path(self, ds_type: str) -> Tuple[str, str]:
        ...
        ats_file_name = self.config.model_name + "_" + self.config.ds_name + '_' + ds_type + "_ats.npy"
        pred_file_name = self.config.model_name + "_" + self.config.ds_name + '_' + ds_type + "_pred.npy"

        return (
            os.path.join(
                self.config.saved_path, ats_file_name
                ,
            ),
            os.path.join(self.config.saved_path, pred_file_name),
        )
    ...
    def _calculate_ats(self, dataset: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        layer_outputs, dnn_output = self._cal_keras_ats(temp_model, dataset)
    ...
    def _cal_keras_ats(self, temp_model: tf.keras.Model, dataset: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        return layer_outputs, dnn_output
...
class LSA(SurpriseAdequacy):
    ...
    def _classification_kdes(self) -> Tuple[Dict[int, gaussian_kde], List[int]]:
        ...
        for label in range(self.config.num_classes):
            ...
            for i in range(row_vectors.shape[0]):
                if np.var(row_vectors[i]) < self.config.min_var_threshold:
                    positions.append(i)
            for p in positions:
    ...
    def _create_gaussian_kde(refined_ats):
        try:
            ...
            double_ats = np.concatenate((refined_ats, refined_ats), axis=1)
            kde = gaussian_kde(double_ats)
        return kde
            ...
            while start < num_targets:
                ...
                for label in range(self.config.num_classes):
                    ...
                    def task(t_batch, t_label, t_start):
                        ...
                        if len(matches) > 0 and t_label in self.class_matrix.keys():
